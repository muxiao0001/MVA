...
| P0-10 | 部分完成 | 真实 `deepseek-v4-flash` 已跑通直接回答、calculator、工具型追问和 `search → todo`；GitHub remote 已存在。当前修改仍需 commit/push，操作录屏仍需人工生成。 | 见 `docs/REAL_API_TEST_EVIDENCE.md`；脱敏机器证据位于本地忽略目录。 |

代码级 P0-01 至 P0-09 已闭环，确定性验收为 **20/20**，真实 API 四条路径通过。因此当前可以判定 MVP 核心功能和代码内安全基线已经完成；`TASK.md` 的最终提交仍需把当前修改发布到代码链接并补齐操作录屏。

### 2026-07-31 系统性复检补充

- 发现含 Key 的本地 `.env` mode 为 `0644`，已立即收紧为 `0600`；Git 忽略规则和跟踪文件秘密扫描均通过。
- TC-04 已升级为两个独立 Python 进程间创建、退出、恢复并读取 todo，不再只重建 Application 对象。
- TC-14 已增加压缩后的真实追问断言，确认早期事实通过非可信 memory 进入模型请求并用于回答。
- Session 标题增加 200 字符和控制字符校验；CLI 输出统一移除 ANSI/OSC 所依赖的终端控制字符。
- 真实 API 新增 `search → todo` 多工具链，验证顺序、副作用落盘和 Mock 声明。
- TC-12 增加 503 重试后成功与 401 不重试的 HTTP Adapter 确定性验证。
- 工具 JSON 拒绝 `NaN`、Infinity 类非标准常量和超过 64 层的嵌套，避免解析资源异常退化为 `internal_error`。
- SQLite 文件改为在首次打开前以 `0600` 安全创建或收紧，并拒绝符号链接和非普通文件。
